...
⚠ **The field name differs by dissector, AND BY CAPTURE — ask for BOTH.** `btatt.value`
for the JBL's GATT; for RFCOMM it is `data.data` *or* `btspp.data` **in the same log**.
Measured 2026-08-26 on one QC35 capture: `btspp` claimed 41 frames where
`btrfcomm && data.len > 4` gave 5 — and later in that same file the vendor app's traffic
appeared under **`data.data` only**, with `btspp` empty across a window holding 131
RFCOMM frames. The dissector claims a channel when tshark witnessed it being set up; a
link already open when the log rotated is undissected and its payload falls back to
`data`. ⚠ **A window that looks silent is the first thing to re-ask with the other
field** — asking only one is how a captured frame reads as "the app sent nothing". A filter that returns 0 rows usually means the wrong field, not a quiet device.
